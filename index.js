...
app.get("/usuarios/:id", (req, res) => {
    try {
        ...
        client.query(
            "SELECT * FROM Usuarios WHERE id = $1", [req.params.id], (err, result) => {
                if (err) {
                    return console.error("Erro ao executar a qry de SELECT id", err);
                }
                if (result.rowCount == 0) {
                    res.send("Usuario com o código " + req.params.id + " não existe no Banco de Dados")
                }
                res.send(result.rows);
                //console.log(result);
            }
            ...
            function (err, result) {
                if (err) {
                    ...
                    if (result.rowCount == 0) {
                        ...
                        res.status(200).json({ info: `Registro excluído. CPF ${id} cancelado` });
                    }
                }
                console.log(result);
            }
